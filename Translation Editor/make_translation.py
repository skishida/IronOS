...
def readTranslations(jsonDir):
    langDict = {}
    UnitDict = {}
    settings_line_single_mode_dict = {}
    ...
    for fileName in os.listdir(jsonDir):
        ...
        if lf.startswith("translation_") and lf.endswith(".json"):
            ...
            # Is settings forced to one line (for large only fonts)
            try:
                settingsIsSingleLine = lang["singleLineSettings"]
            except KeyError:
                settingsIsSingleLine = False
            ...
            settings_line_single_mode_dict[langCode] = settingsIsSingleLine
    return langDict, UnitDict, settings_line_single_mode_dict
...
def writeUnit(languageCode, defs, f, UnitCodes, settings_single_line_codes):
    print("Generating unit block for " + languageCode)
    lang = langDict[languageCode]
    unit = UnitDict[UnitCodes]
    settings_single_line = settings_single_line_codes[languageCode]
    try:
        langName = lang["languageLocalName"]
    except KeyError:
        langName = languageCode
    ...
    if unit:
        ...
        f.write(to_unicode("    //#define  ENABLED_FAHRENHEIT_SUPPORT" + "\n"))
    if settings_single_line:
        f.write(to_unicode("    #define  SETTINGS_USES_SINGLE_LINE" + "\n"))
    else:
        f.write(to_unicode("    //#define  SETTINGS_USES_SINGLE_LINE" + "\n"))
...
def writeTarget(
    outFileTranslationCPP,
    outFileUnitH,
    defs,
    langCodes,
    UnitCodes,
    settings_single_line_codes,
):
    ...
    with io.open(outFileUnitH, "w", encoding="utf-8", newline="\n") as f:
        ...
        for langCode, UnitCode in zip(langCodes, UnitCodes):
            writeUnit(langCode, defs, f, UnitCode, settings_single_line_codes)
        f.write(to_unicode("\n#endif /* _UNIT_H */\n"))
...
if __name__ == "__main__":
    ...
    langDict, UnitDict, settings_is_single_line = readTranslations(jsonDir)
